"""Generate many random, valid traffic-light configurations.

Each configuration is written to ``config_XXXX.json`` in the output directory.
Generation is fully deterministic given ``--seed`` so runs are reproducible.
...
def make_config(config_id, rng, traffic_level, simulation_duration, base_seed):
    """Build one valid configuration dictionary.

    The green split is sampled so that the hard constraint always holds::

        green_ns + green_ew + 2*yellow + 2*all_red <= cycle_length
    ...
    # Budget of green seconds to split between NS and EW
...
def generate_configs(n_configs, output_dir, seed, traffic_level, simulation_duration):
    """Generate ``n_configs`` configuration files and return their paths."""
    ensure_dir(output_dir)
    rng = random.Random(seed)
    paths = []
    for config_id in range(n_configs):
        config = make_config(
            config_id, rng, traffic_level, simulation_duration, seed
        )
